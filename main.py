...
def max_similar_triangle(ab2, bc2, ac2, n):
    ...
    while d:
        ...
        if judge_square_sum(max_ab2, n) is False or judge_square_sum(max_bc2, n) is False:
            continue
...
def main():
    try:
        while True:
            try:
                ab2, bc2, ac2, n = map(int, input("请输入三角形三边的平方数和正方形的格数（空格隔开）：").split())
                ...
                if b_poinx is False or c_point is False:
                    ...
                    print(f"a[0,0], b[{b_poinx}], c[{c_point}]")
